...
// Async function to handle DELETE requests, removing the medias for a given ingress key
const handleDelete = async (ctx: HandlerContext): Promise<APIGatewayProxyResult> => {
    // const key = ctx.event.pathParameters?.key;
    //Get key from url params instead
    const key = ctx.event.queryStringParameters?.key; 

    if (!key) {
        return {
            statusCode: 400,
            headers: defaultHeaders(),
            body: JSON.stringify({ message: 'Invalid request path (key)' }),
        };
    }
    ...
    const participantCodes = getResult.Item.participantCodes;

    // 2. Delete the item from the imageMetadataTable
    const deleteParams = {
        TableName: ctx.imageMetadataTable,
        Key: { ingressKey: key },
    };
    ...
    // 3. Update the participantMetadataTable for participants to remove the key
    const removePromises = participantCodes.values.map((code: string) => {
        const removeParams = {
            TableName: ctx.participantMetadataTable,
            Key: { participantCode: Number(code) },  // Ensure correct type
            UpdateExpression: 'DELETE ingressKeys :key',
